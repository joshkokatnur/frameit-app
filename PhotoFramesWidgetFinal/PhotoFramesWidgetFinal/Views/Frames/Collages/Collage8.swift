            ...
            if preview {
                ZStack {
                    ...
                    VStack(spacing: geo.size.width / 30) {
                        ...
                        HStack(spacing: geo.size.width / 30) {
                            CroppedImage(image: images[2], transform: transforms[2], scaleFactor: scaleFactors[2], geoSize: CGSize(width: geo.size.width / 2 - (geo.size.width / 20), height: geo.size.height / 2 - (geo.size.width / 20)))
                                .frame(width: geo.size.width / 2 - (geo.size.width / 20), height: geo.size.height / 2 - (geo.size.width / 20))
                                .clipShape(RoundedRectangle(cornerRadius: 18, style: .continuous))
                                .innerShadow(radius: 10, color: UIColor.black.withAlphaComponent(0.2))
                                .clipShape(RoundedRectangle(cornerRadius: 18, style: .continuous))
                            
                            CroppedImage(image: images[3], transform: transforms[3], scaleFactor: scaleFactors[3], geoSize: CGSize(width: geo.size.width / 2 - (geo.size.width / 20), height: geo.size.height / 2 - (geo.size.width / 20)))
                                .frame(width: geo.size.width / 2 - (geo.size.width / 20), height: geo.size.height / 2 - (geo.size.width / 20))
                                .clipShape(RoundedRectangle(cornerRadius: 18, style: .continuous))
                                .innerShadow(radius: 10, color: UIColor.black.withAlphaComponent(0.2))
                                .clipShape(RoundedRectangle(cornerRadius: 18, style: .continuous))
